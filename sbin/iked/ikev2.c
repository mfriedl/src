    ...
    struct ike_header *hdr)
{
	...
	if (env->sc_half_open_count < env->sc_cookies_threshold) {
		/* discard unused secret */
		if (env->sc_half_open_count == 0 &&
		    env->sc_cookies_secret != NULL) {
			ibuf_free(env->sc_cookies_secret);
			env->sc_cookies_secret = NULL;
		}
		return (0);
	}
	...
	if (env->sc_cookies_secret == NULL) {
		env->sc_cookies_secret = ibuf_random(IKED_NONCE_SIZE);
		if (env->sc_cookies_secret == NULL) {
	...
	if ((ctx = EVP_MD_CTX_new()) == NULL ||
	    ...
	    EVP_DigestUpdate(ctx, ibuf_data(env->sc_cookies_secret),
	    ibuf_length(env->sc_cookies_secret)) != 1 ||
